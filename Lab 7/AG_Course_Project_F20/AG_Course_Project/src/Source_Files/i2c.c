...
static uint32_t scheduled_write_cb;
//***********************************************************************************
// Global functions
//***********************************************************************************
...
void i2c_start(I2C_TypeDef *i2c_peripheral, uint32_t slave_address, uint32_t command, uint32_t *read_value, uint32_t callback, bool function, uint32_t bytes, uint32_t write_value){
	...
	i2c_peripheral_state.function = function;
	i2c_peripheral_state.read_bytes = bytes;
	i2c_peripheral_state.write_value = write_value;
	*i2c_peripheral_state.read_value = 0;


	if(function){
		scheduled_write_cb = callback;
	}
	else{
		scheduled_read_cb = callback;
	}
...
static void i2c_ack_interrupt(void){
	switch(i2c_peripheral_state.state){
		...
		case send_cmd:
			i2c_peripheral_state.peripheral->CMD |= I2C_CMD_CLEARTX;
			if(i2c_peripheral_state.function){
				i2c_peripheral_state.peripheral->TXDATA |= i2c_peripheral_state.write_value;
				i2c_peripheral_state.state = write_data;
			}
			else{
		...
		case read_request:
			i2c_peripheral_state.peripheral->CMD |= I2C_CMD_CLEARTX;
			i2c_peripheral_state.state = read_byte;
			break;
		case read_byte:
			break;
		case write_data:
...
static void i2c_mstop_interrupt(void){
	switch(i2c_peripheral_state.state){
		...
		case stop_comm:
			sleep_unblock_mode(EM2);
			if(i2c_peripheral_state.function){
				add_scheduled_event(scheduled_write_cb);
			}
			else{
...
static void i2c_rxdatav_interrupt(void){
	switch(i2c_peripheral_state.state){
		...
		case read_byte:
			i2c_peripheral_state.peripheral->CMD |= I2C_CMD_CLEARTX;
			*i2c_peripheral_state.read_value |= (i2c_peripheral_state.peripheral->RXDATA << (8 * (i2c_peripheral_state.read_bytes - 1)));
			i2c_peripheral_state.read_bytes--;
			if(i2c_peripheral_state.read_bytes == 0){
...
/***************************************************************************//**
 * @brief
 *	Sets the state of the i2c peripheral to busy.
 *
 * @details
 *	Sets the i2c state machine as busy to allow the transmission of all the bytes to the peripheral
 *
 *
 ******************************************************************************/
...
bool i2c_busy(void){
	return i2c_peripheral_state.sm_busy;
}
